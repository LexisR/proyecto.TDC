// esta función debe validar si el n° de tarjeta es valida
function isValidCard() { 
  var consultaTdc = prompt("Hola introduzca su numero de tarjeta"); // consultar el numero de tdc
  var numerosPares = []; // creamos un array para guardar solo los numeros en posicion par de tarjeta invertida
  var numerosImpares = [];
  ...
  for (var j = 0; j <= numerosPares.length - 1; j++) {
    var multiplicacion = numerosPares[j] * 2;  // colocar el nmbre d ela variable para acceder
    if (multiplicacion >= 10) {
      // sumar digitos, como saco los digitos de un numero.
      numerosPares[j] = parseInt((multiplicacion % 10) + (multiplicacion / 10));
    } else {
      numerosPares[j] = multiplicacion;
    }
  ...
  if (resultado % 10 === 0) {
    return alert("Tu numero de tarjeta es valido, Bienvenido a mi web");
  } else {
...
isValidCard() // siempre debemos invocar a la función 
